...
`run_csv` reads OHLCV **bars**: one header line, then `timestamp,open,high,low,close,volume`. Only the timestamp and close columns are used; each bar is replayed as one trade, so `on_trade` fires and `on_bar` does not. Timestamps below `1e12` are treated as seconds and scaled to nanoseconds; `run_ohlcv(ts, close, symbol)` applies the same magnitude-based guess to its `ts` array, since both read timestamps of an unknown, caller-supplied unit. `run_bars(...)` (full OHLC, fires `on_bar`) is different: its `start_time_ns` / `end_time_ns` arrays are nanoseconds by contract, so it uses them exactly as given, with no unit guessing at all. Other entry points: `run_tape(path)` and `run_tapes(paths)` for `.floxlog` tapes.
