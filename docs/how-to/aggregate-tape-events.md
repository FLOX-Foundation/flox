...
`DataReader.run([...])` walks a `.floxlog` once and forwards every event to a panel of streaming aggregators. The aggregators run in C++ without per-event Python or JS crossings, so adding more aggregators to the panel only adds a small per-event cost; the panel is walked once regardless.
...
For each `window_ns`, the aggregator records the sliding-window event count at every event arrival, builds a histogram of those observations, and at `finalize()` resolves each quantile `q` to the smallest count value `C` for which `fraction(observed ≤ C) ≥ q`.
...
`onEvent` is called once per event per pass. Inside the dispatch loop each aggregator adds one indirection per event; the cost scales with the panel size but not with how many passes you'd otherwise need.

The aggregators are self-contained: no I/O, no Python crossings, no hot-path allocations beyond the bounded sliding deques in Peak and Quantile.
...
The Quantile histogram is sparse (`unordered_map<count_value, occurrences>`). Distinct count values are bounded by the peak burst size (usually dozens), while observations scale with the event count, so memory stays in the kilobytes even on multi-gigabyte captures.
