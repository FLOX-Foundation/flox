...
| `FLOX_ENABLE_ONNX` | OFF | Compile the ONNX Runtime inference nodes (`flox/ml/`) into the core library, and the `test_onnx_inference` suite once tests are on. Hard-fails at configure time if the onnxruntime library/headers are not found — no bundled fallback. **Not exercised by any CI workflow**: it needs the onnxruntime C++ SDK installed on the runner, which none of them do. Verify locally with `-DFLOX_ENABLE_ONNX=ON -DFLOX_BUILD_TESTS=ON` after installing onnxruntime. |
| `FLOX_ENABLE_AF_XDP` | OFF | Compile the AF_XDP receive-path backend (`flox/net/af_xdp_receive_path.h`). Linux-only; hard-fails at configure time if libxdp/libbpf are not found. **Not exercised by any CI workflow** — no test targets are gated on this flag today, and none of the runners carry the kernel/library support it needs. |
| `FLOX_ENABLE_TRACY` | OFF | Link the Tracy profiler client. Adds runtime instrumentation; OFF in production builds. **Not exercised by any CI workflow** — no test targets are gated on this flag. |
| `FLOX_ENABLE_CPU_AFFINITY` | OFF | Compile pthread / NUMA-aware affinity helpers. Linux only; on macOS / Windows the calls become no-ops. **Warning:** can hurt perf on busy or shared systems — only use on isolated dedicated hardware. The gated `test_cpu_affinity` suite runs in the `affinity-tests` CI job, but only on the weekly schedule (`cron: '0 3 * * 0'`), not on every pull request. |
