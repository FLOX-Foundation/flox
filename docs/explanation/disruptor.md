...
### Sleeping until the earlier of two things

A driver rarely only waits for a publisher. It sweeps, it watches a threshold,
it has periodic business of its own -- and what it wants is to sleep until the
*earlier* of "somebody published" and "the next thing is due". `parkUnless`
bounds the sleep only by the net, and a 50 ms net under a cadence measured in
single-digit milliseconds is not a net, it is the schedule.

```cpp
int64_t nextSweep = flox::venue::venueMonoNs();

while (running) {
  const int64_t now = flox::venue::venueMonoNs();
  if (now >= nextSweep) {
    nextSweep = now + cadenceNs;
    sweep();
  }
  if (step()) continue;
  set.parkUntil(nextSweep, [&] { return !running || pending(); });
}
```

`parkUntil(deadlineNs, pred)` is the same sleep under the same discipline,
with a ceiling on it: it returns on a publish, on the predicate, or on the
deadline -- whichever is first, and the caller loops either way, because a
return says only "look again" and never why. `deadlineNs` is on the steady
clock; a deadline already in the past is "look once and come straight back".
`parkUnless(pred)` is `parkUntil(WakeSet::kNoDeadline, pred)`.
...
```cpp
// evs is a contiguous array, 1 <= count <= CapacityPow2 / 2
int64_t lastSeq = tradeBus->publishBatch(evs, count);
```

The bound is enforced, not assumed: an empty batch and one larger than half the
ring are both refused with -1, in every build, and nothing is published. It is
the ring's bound rather than a style rule -- a range wider than the ring
reserves slots it wraps back onto, so the wrap gate would wait on sequences
inside the same batch that only this publisher can stamp, and the publisher
would never return. The refusal is always a return.
...
`stop()` closes publishing, waits out the publishers that are already inside
the ring, joins every consumer thread, destroys whatever events the ring still
holds, and puts the gating lines back where a fresh bus starts. A restarted bus
numbers its events from zero again, so sequence numbers are unique within a run,
not across runs. Anything that persists a `tickSequence` past a restart needs
its own run identifier.

A publish racing `stop()` has one of two outcomes, and the publisher can tell
them apart from the return value: it is refused (-1, or `STOPPED`), or the event
is in the ring and the shutdown will not throw it away before the consumers that
were going to get it have had their chance. There is no third outcome where the
bus accepted an event, handed back a sequence number, and dropped it on the way
down -- which is a loss the publisher had no way to learn about.

Why a flag cannot deliver that: a publisher reads "is the bus running" once, on
the way in, and can be preempted between that read and its claim for as long as
the scheduler likes, so by the time it constructs into a slot the answer may be
hours old. What `stop()` closes instead is the sequence line itself, in one
atomic step: every claim taken afterwards comes back refused, and every claim
taken before it is one `stop()` can count and wait for. The publish path pays
nothing for this -- it announces nothing on the way in, and the counter it bumps
on the way out is the one it already bumped.

So `stop()` returns only when no publisher is writing into a slot, which is what
makes it safe to destroy or restart the bus. It waits for a publisher parked
inside the event's own copy constructor, and it does not wait for one parked at
the wrap gate or the reclaim fence: those give up instead, so a stalled consumer
cannot keep a shutdown from finishing.
...
- Stalls. `checkHealth()` sweeps all consumers and fires the callback with
  ...
  health thread or set `enableMonitorThread` to poll automatically, on a
  period of `monitorPeriod(stallThreshold)` -- half the threshold, floored at
  1 ms so that a sub-millisecond threshold cannot turn the sweep into a spin.
  One sweeping thread at a time; the bookkeeping is not synchronized between
  checkers.
...
The callback is edge-triggered. For the level-triggered question -- who is
unhealthy right now -- `consumerHealth(i)` and `healthSnapshot()` read what the
last sweep recorded, from any thread, while the sweep is running and after the
bus has stopped. `consumerHealthReport(i)` adds the two numbers a supervisor
needs to act: the sequence the consumer had reached and the instant it last
moved, taken from the same sweep as the state. A report that mixed the sequence
from one sweep with the instant from another would describe progress that never
happened, so `lastChange` moves only together with `lastSeen`.
